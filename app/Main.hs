...
sheetDocs :: Int -> Int -> Double -> Double -> [Document] -> [Document]
sheetDocs columns rows w h symbolDocs = let
    radius = w
...
main = do
    ...
    traverse_ (\(i, doc) -> renderPng ("deck/sheet" <> show i <> ".png") doc) (zip [1..] (sheetDocs 3 4 3000 3000 (Prelude.take 31 (cycle symbolDocs))))
        where
